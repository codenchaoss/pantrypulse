...
class SupplierService:
    ...
    def generate_supplier_message(
        ...
        clean_restaurant = restaurant_name if (restaurant_name and "Bistro" not in restaurant_name) else "PantryPulse Restaurant"
        clean_urgency = urgency_level.capitalize() if urgency_level else "Medium"

        # Invoke the hybrid orchestration pipeline with strict corporate procurement email formatting rules
        question = (
            f"Draft a formal, highly professional procurement purchase request email for supplier: {clean_supplier} Team "
            f"from {clean_restaurant} for ingredient: {ingredient_display}, quantity: {clean_quantity}, priority: {clean_urgency}, required before date: {clean_date}.\n"
            "Format the email message body using this exact corporate structure:\n"
        ...
        if not message_body:
            ...
            message_body = self._get_fallback_message(
                ...
                clean_date, 
                detected_lang,
                restaurant_name=clean_restaurant,
                contact_person=contact_person,
                urgency_level=clean_urgency
            )
    ...
    def _get_fallback_message(
        ...
        date: str, 
        language: str,
        restaurant_name: str = "PantryPulse Restaurant",
        contact_person: str = "Procurement Team",
        ...
        """
        lang = language.lower()
        clean_supplier = supplier if supplier and supplier != "Supplier" else "Supplier"
        clean_rest = restaurant_name if (restaurant_name and "Bistro" not in restaurant_name) else "PantryPulse Restaurant"
        clean_urgency = urgency_level.capitalize() if urgency_level else "Medium"
        
        if "telugu" in lang:
            return (
                f"ప్రియమైన {clean_supplier} బృందానికి,\n\n"
                f"{clean_rest} నుండి శుభాకాంక్షలు.\n\n"
                f"మేము బాగా ఉన్నారని ఆశిస్తున్నాము.\n\n"
                f"మా ప్రస్తుత ఇన్వెంటరీ అవసరాల ఆధారంగా, మేము ఈ క్రింది వస్తువు కొనుగోలు అభ్యర్థనను తెలియజేస్తున్నాము:\n\n"
                f"Purchase Details\n\n"
                f"• Ingredient      : {ingredient}\n"
                f"• Quantity        : {quantity}\n"
                f"• Priority        : {clean_urgency}\n"
                f"• Required Before : {date}\n\n"
                f"దయచేసి సరుకు అందుబాటుతో పాటు అంచనా వేసిన డెలివరీ సమయాన్ని ధృవీకరించగలరు. స్టాక్ లభ్యత లేదా డెలివరీ సమయాల్లో మార్పులు ఉంటే తెలియజేయగలరు.\n\n"
                f"మీ నిరంతర సహకారానికి ధన్యవాదాలు.\n\n"
                f"భవదీయులు,\n\n"
                f"Procurement Team\n"
                f"{clean_rest}\n\n"
                f"──────────────────────────────────────\n"
                f"Generated by PantryPulse AI Procurement Assistant"
            )
        else:
            return (
                f"Dear {clean_supplier} Team,\n\n"
                f"Greetings from {clean_rest}.\n\n"
                f"We hope you are doing well.\n\n"
                f"Based on our current inventory requirements, we would like to place a purchase request for the following item.\n\n"
                f"Purchase Details\n\n"
                f"• Ingredient      : {ingredient}\n"
                f"• Quantity        : {quantity}\n"
                f"• Priority        : {clean_urgency}\n"
                f"• Required Before : {date}\n\n"
                f"Kindly confirm the availability of the requested item along with the expected delivery schedule. If there are any changes regarding stock availability, pricing, or delivery timelines, please let us know so that we can plan our kitchen operations accordingly.\n\n"
                f"We sincerely appreciate your continued partnership and look forward to your confirmation.\n\n"
                f"Kind Regards,\n\n"
                f"Procurement Team\n"
                f"{clean_rest}\n\n"
                f"──────────────────────────────────────\n"
