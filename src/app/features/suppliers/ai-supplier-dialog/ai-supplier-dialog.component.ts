...
export class AiSupplierDialogComponent implements OnInit {
 ...
 copyRequest() {
  ...
  navigator.clipboard.writeText(this.aiResponse.message);

  this.toast.success(
    'PantryPulse',
    'Purchase request copied successfully.'
  );
...
downloadPdf(): void {
  ...
  doc.text('PantryPulse', 20, 20);

  doc.setFontSize(16);
  doc.setTextColor(40);
  doc.text('AI Procurement Request', 20, 32);
  ...
  doc.text(
    `Supplier: ${this.aiResponse.supplierName}`,
    20,
    48
  );

  doc.text(
    `Order Reference: ${this.aiResponse.orderId}`,
    20,
    58
  );
  ...
  doc.text(
    `Restaurant: ${this.supplierForm.value.restaurant_name}`,
    20,
    78
  );

  doc.text(
    `Subject: ${this.aiResponse.subject}`,
    20,
    88
  );
  ...
  // Message
  doc.setFontSize(13);
  doc.text("Generated Purchase Request", 20, 105);
  ...
  doc.save(
      `Purchase_Request_${this.aiResponse.orderId}.pdf`
  );
